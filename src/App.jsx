...
function BodegaCard({ name, items, icon: Icon, active, onClick, onNameChange, onDelete }) {
  ...
  return (
    <div
      ...
      className={`relative flex flex-col items-center justify-center p-6 bg-dark-surface border rounded-2xl cursor-pointer transition-all hover:shadow-lg hover:shadow-brand-red/10 ${active ? 'border-brand-red ring-1 ring-brand-red bg-dark-bg3' : 'border-dark-border hover:border-brand-red/50'}`}
    >
      <div className="absolute top-4 right-4 flex items-center gap-2">
        ...
        <div className="flex items-center text-xs font-medium text-text-muted bg-dark-bg px-2 py-1 rounded-md border border-dark-border">
          <svg className="w-3.5 h-3.5 mr-1 text-brand-cyan" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2-2z"></path></svg>
          {items}
        </div>
      </div>
      <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${active ? 'bg-gradient-to-br from-brand-red/20 to-brand-ember/20 text-brand-ember border border-brand-red/30' : 'bg-dark-bg text-text-muted border border-dark-border'}`}>
        {Icon ? <Icon /> : <Icons.Inventory />}
      </div>
      {isEditing ? (
        <div className="flex items-center gap-2 w-full mt-1" onClick={e => e.stopPropagation()}>
          <input autoFocus type="text" value={tempName} onChange={e => setTempName(e.target.value)} className="w-full px-2 py-1 text-sm bg-dark-bg2 border border-brand-cyan rounded text-white focus:outline-none text-center rajdhani" />
        ...
        <h3 className={`text-sm font-semibold text-center rajdhani text-lg ${active ? 'text-white' : 'text-text-main'}`}>{name}</h3>
      )}
    </div>
  );
}
    ...
    <div className="flex flex-col h-screen bg-dark-bg text-text-main overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-dark-bg relative" onClick={() => showProfileMenu && setShowProfileMenu(false)}>
          ...
          <div className="flex justify-between items-center px-8 py-4 border-b border-dark-border bg-dark-bg2 z-10 flex-shrink-0">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-dark-bg flex items-center justify-center text-brand-cyan border border-dark-border shadow-[0_0_10px_rgba(56,189,248,0.1)]">
                {activeTab === 'catalogo' ? <Icons.Traceability /> : <Icons.Inventory />}
              </div>
              <h2 className="text-lg font-bold text-white rajdhani tracking-wide">
                {activeTab === 'bodegas' ? 'Ubicaciones Principales' : activeTab === 'catalogo' ? 'Catálogo de Materiales' : 'Dashboard'}
              </h2>
            </div>
            <div className="flex items-center gap-3">
              {activeTab === 'bodegas' && (
              ...
              {activeTab === 'catalogo' && (
                <>
                  ...
                  <button className="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-brand-red to-brand-ember rounded-lg hover:opacity-90 transition-colors flex items-center gap-2 shadow-[0_4px_15px_rgba(232,55,42,0.3)]">
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            <div className="p-8">
              <div className="flex gap-4 mb-6">
                <div className="flex-1 relative">
                  ...
                  <input type="text" placeholder="Buscar por nombre..." className="w-full pl-10 pr-4 py-2 bg-dark-bg3 border border-dark-border text-white rounded-lg outline-none focus:border-brand-cyan focus:ring-1 focus:ring-brand-cyan transition-all text-sm placeholder-text-muted" />
                </div>
                <div className="w-64 relative">
                  <select className="w-full px-4 py-2 bg-dark-bg3 border border-dark-border text-white rounded-lg outline-none focus:border-brand-cyan focus:ring-1 focus:ring-brand-cyan appearance-none text-sm">
                ...
                <table className="w-full text-left text-sm">
                  <thead className="bg-dark-bg2 border-b border-dark-border text-text-muted font-medium rajdhani text-base">
                    <tr>
                      ...
                      <th className="px-6 py-4 font-semibold text-center">Req. Mantención</th>
                      <th className="px-6 py-4 font-semibold text-right">Acciones</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-dark-border">
                    <tr className="hover:bg-dark-bg3 transition-colors">
                      <td className="px-6 py-4 font-medium text-white">Manguera 50mm</td>
                      <td className="px-6 py-4"><span className="px-2.5 py-1 bg-brand-cyan/10 border border-brand-cyan/20 text-brand-cyan rounded-full text-xs font-medium">Extinción</span></td>
                      <td className="px-6 py-4 text-center text-text-muted">—</td>
                      <td className="px-6 py-4 text-center text-brand-green">✓</td>
                      <td className="px-6 py-4 text-center text-brand-green">✓</td>
                      <td className="px-6 py-4 text-right">
                        <button className="text-text-muted hover:text-brand-cyan transition-colors mr-3"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg></button>
                        <button className="text-text-muted hover:text-brand-red transition-colors"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                      </td>
                    </tr>
                    <tr className="hover:bg-dark-bg3 transition-colors">
                      <td className="px-6 py-4 font-medium text-white">Guantes de Rescate</td>
                      <td className="px-6 py-4"><span className="px-2.5 py-1 bg-brand-cyan/10 border border-brand-cyan/20 text-brand-cyan rounded-full text-xs font-medium">EPP</span></td>
                      <td className="px-6 py-4 text-center text-brand-green">✓</td>
                      <td className="px-6 py-4 text-center text-text-muted">—</td>
                      <td className="px-6 py-4 text-center text-text-muted">—</td>
                      <td className="px-6 py-4 text-right">
                    ...
                    <tr className="hover:bg-dark-bg3 transition-colors">
                      <td className="px-6 py-4 font-medium text-white">Pitón neblinero</td>
                      <td className="px-6 py-4"><span className="px-2.5 py-1 bg-brand-cyan/10 border border-brand-cyan/20 text-brand-cyan rounded-full text-xs font-medium">Extinción</span></td>
                      <td className="px-6 py-4 text-center text-text-muted">—</td>
                      <td className="px-6 py-4 text-center text-brand-green">✓</td>
                      <td className="px-6 py-4 text-center text-brand-green">✓</td>
          ...
          {activeTab !== 'bodegas' && activeTab !== 'catalogo' && activeTab !== 'vehiculos' && (
